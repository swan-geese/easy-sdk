package com.easy.sdk.common.extra.mybatis.util;
...
import com.easy.sdk.common.extra.mybatis.annotation.QueryCondition;
import com.easy.sdk.common.extra.mybatis.common.ApplySql;
import com.easy.sdk.common.extra.mybatis.common.FuncExps;
import com.easy.sdk.common.extra.mybatis.common.SqlTemplate;
import com.easy.sdk.common.extra.mybatis.constant.MybatisConstant;
...
public interface MybatisUtil {
	...
	static <T> void parseApplySql(AbstractWrapper<T, ?, ?> wrapper, Object v) {
		if (v instanceof ApplySql) {
			...
			ApplySql.ApplySqlSegment normalSegment = as.getNormalSegment();
			List<String> groupByList = as.getGroupByList();
			List<String> orderByList = as.getOrderByList();
			ApplySql.ApplySqlSegment havingSegment = as.getHavingSegment();
			// 自定义sql
	...
	static Object[] getValueArray(List<ApplySql.ParamValue> list) {
	...
	static <T> ApplySql buildApplySql(LambdaQueryWrapper<T> wrapper, Class<T> beanClass) {
		// sql片段
		ApplySql.ApplySqlSegment normalSegment = null;
		List<String> groupByList = Collections.emptyList();
		ApplySql.ApplySqlSegment havingSegment = null;
	 ...
	 * @return {@link ApplySql.ApplySqlSegment}
	 */
	static ApplySql.ApplySqlSegment buildApplySqlSegment(AbstractISegmentList segmentList, Map<String, Object> valuePairs) {
		// 获取sql片段
		...
		List<ApplySql.ParamValue> valueList = null;
		// 获取占位符中的key
		...
		if (CollUtil.isNotEmpty(keyList)) {
			valueList = CollUtil.map(keyList, k -> {
				...
				return ApplySql.ParamValue.of().setType(type).setValue(v);
			}, true);
		}
		// sql 处理
		sqlSegment = replacePlaceholder(
				sqlSegment.replaceAll("#\\{ew.paramNameValuePairs.MPGENVAL[0-9]+\\}", MybatisConstant.ASK));
		return ApplySql.ApplySqlSegment.of().setSqlSegment(sqlSegment).setValueList(valueList);
	}
